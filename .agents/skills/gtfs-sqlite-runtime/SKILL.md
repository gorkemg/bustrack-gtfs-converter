...
- The converter derives a project-specific `canonical_stop_counterparts` table (schema_version >= 1.2): per `(route_id, direction_id)` the opposite-direction twin of each stop, for "switch direction" UI. Absence of a row means no counterpart exists (e.g. loop routes).
...
- Do not assume release discovery via "latest". Use explicit tags such as `pvta`, `pvta-previous`, or `pvta-prerelease`. The `pvta-prerelease` tag is a beta-channel artifact built from the `beta` branch and must not be given to production clients. Archive snapshots are untagged maintainer-only draft releases and cannot be downloaded via tag URLs.
